        ...
        <div style={{backgroundColor: '#0C0C0C'}} className='text-white w-[50vmax] h-[35vmax] rounded-[0.5vmax] overflow-auto flex flex-col justify-start items-center'>
            ...
            <div className='w-full p-[0.5vmax] justify-center text-center items-center my-[2vmax] text-[1.3vmax] font-semibold'>
                ...
                <div className='w-full my-[1vmax] flex justify-center gap-[2vmax] items-center'><button onClick={(e)=>{
                    e.preventDefault();
                    saveFunc(type);
                }} style={{backgroundColor: '#F2613F'}} className='p-[0.5vmax] rounded-[0.5vmax] text-[1.1vmax] font-bold'>{profileInfo.saved.includes(info._id) ? 'Remove From Favorites' : 'Save To Favorites' && profileInfo.savedApi.includes(info.id) ? 'Remove From Favorites' : 'Save To Favorites'}</button><button style={{backgroundColor: '#F2613F'}} className='p-[0.5vmax] text-[1.1vmax] font-bold rounded-[0.5vmax]'>Add To Purchase</button></div>
        </div>
        <div onClick={(e)=>{
            e.preventDefault();
            setShowModal(false);
